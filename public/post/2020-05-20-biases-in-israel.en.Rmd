...
image: '' #post\2020-05-20-biases-in-israel.en_files\israel_flag_s.png 
...
* Self-Reported Direct Preference: Preference between the dominant and non-dominant category (scale: 1 "I strongly prefer NON-DOMINANT CATEGORY to DOMINANT CATEGORY" to 7 "I strongly prefer DOMINANT CATEGORY to NON-DOMINANT CATEGORY". 4 indicates No preference).
* Feeling Thermometer: Cold to warm feelings toward category (scale: 0 "very cold” 1 to 10 "very warm"). We computed a difference score of feeling towrard DOMINANT CATEGORY - feeling toward NON-DOMINANT CATEGORY.

The tasks also included secondary measures such as perception measures and demographic items. You can check out the data on OSF! **INSERT LINK HERE**
...
The results from our analyses could be used as a first step in the examination of implicit and explicit bias toward groups Israel and of the most prominent bias in this country (that is, the category with the largest difference between the two groups). This would be a first step as Project Implicit's samples are *not* representative samples. 
...
*Note*: The plot below shows the mean IAT score for each test topic.
...
Future studies could explore whether the extent of implicit bias as measured with the IAT relates to the extent of real-world discrimination and other negative treatment.


Israelis were often biased *explicitly*, and not just implicitly... but, as might be expected (e.g., due to self-presentation bias), this bias was weaker in its magnitude, compared with the implicit bias. 
...
*Note*: The plot below shows the mean self reported direct preference score for each test topic. 
Not all tasks included the direct self-reported preference measure.
...
*Note*: The plot below shows the mean feeling thermometer difference score for each test topic. Not all tasks included the feeling thermometer measure.
...
Does that mean that Israelis are more OK with being biased toward Fat people, and in favor of their own country, than toward elderly people? Does that relate to certain social values in the Israeli society? We think these are interesting questions that could be adressed in future studies. 
...
What we found is that bias, both implicit and explicit stays pretty much the same troughout the 12 years. Across all years, we can see that explicit and implicit judgmetns favor the dominant groups, and that implict bias is much larger.

```{r}

# drop NAs and set as factor
...
*Note*: Each plot shows the distribution of scores, with mean + 95% CI, and 0.25, 0.50, and 0.75 quantiles. The dashed line marks the point of no preference between the two groups.
...
font("legend.text", size = 20, color = "black", face = "bold")+
...
We can see that, indeed, the implicit bias varies across the different ethnicities, and is weaker for Mizrahi Israelis. Still, *all ethnicity groups* in our Israeli sample (including Mizrahi Israelis) show the same direction of bias.
