...
**Based on the participants' reaction-times and errors, the IAT score on each of these tasks reflects the average strength of association between the categories and the attributes used in the task (for example, an association between Young people + Good and Old people + Bad).** Some researchers interpret this strength of association as the participants' implicit or automatic preference between the two categories/ an automatic bias in judgment (but see disclaimer below). Here we take a conservative stand, and thus refer to IAT scores in a certain direction (rather than zero) as *bias in IAT scores*.
...
We found a robust and persistent pro-dominant association: The IAT scores from all tasks revealed that Israelis **strongly** associate the dominant social groups (e.g., White people, Ashkenazi people, Young people) with positive attributes and the non-dominant social groups (Black people, Mizrahi people, Old people) with negative attributes.
The results from our analyses could be used as a first step in the examination of (supposedly) automatic bias toward groups Israel and of the most prominent bias in this country (that is, the category with the largest difference between the two groups). This would be a first step as Project Implicit’s samples are not representative samples, and because it would be preferable to test the generalizability of automatic bias beyond the IAT scores.


## IAT Scores per Task  

All tasks point to a pro-dominant > association. The strongest bias among Israeli participants is regarding **Nationalism (Own Country > USA)**, **Age (Young > Old)**, and **Gender-Science stereotypes (Male+Science - Female+Liberal arts > Male+Liberal arts - Female+Science)**, and the smallest bias is regarding **Sexuality (Straight > Gay people)** and **Weight (Thin > Fat)**:

*Note*. The plot below shows the effect-size (Cohen's d) as a function of the IAT task. Larger effect-sizes indicate stronger pro-dominant bias.
...
Future studies could explore whether the extent of bias as measured with the IAT relates to the extent of real-world discrimination and other negative treatment.

## Self-Report Scores per Task

Israelis were often biased *in their self-reports*, and not just in their IAT scores... but, as might be expected, this bias was a bit weaker in its magnitude. Interestingly, there were some differences between the strongest bias in the IAT scores and in the self-report scores. Here, the strongest self-reported bias among Israeli participants is regarding **Nationalism (Own Country > USA)** and **Weight (Thin > Fat)**, and the smallest bias is regarding **Sexuality (Straight > Gay people)**, **Ethnicity (Ashkenazi > Mizrahi)**, and **Age (Young > Old)**:

### Self-Reported Direct Preference

*Note*: The plot below shows the effect-size (Cohen's d) as a function of the direct self-reported preference task. Larger effect-sizes indicate stronger pro-dominant bias. Not all tasks included the direct self-reported preference measure.
Gender-Science: a difference score. Higher scores indicate a consideration of science as masculine and liberal arts as feminine.
...
The effect-sizes are smaller on this measure, compared with the previous ones. This difference might result from the nature of each measure: while the Self-reported preference measure *directly* compares the two categories (as does the IAT), the thermometer *does not* compare the two categories directly. A comparative context might facilitate bias.

*Note*: The plot below shows the effect-size (Cohen's d) as a function of the feeling thermometer task. Larger effect-sizes indicate stronger pro-dominant bias. Not all tasks included the feeling thermometer measure.
...
What we found, is that bias, both in IAT scores and in self-report stays the same throughout the 12 years. Across all years, we can see that self-report and IAT scores favor the dominant groups, and that the bias reflected in the IAT scores is much larger.
...
pp <- ggline(iat_df, x = "Year", y = "Cohens_d", color = "Task", size = 2)
...
pp <- ggline(att_df, x = "Year", y = "Cohens_d", color = "Task", size = 2)
...
pp <- ggline(thermo_df, x = "Year", y = "Cohens_d", color = "Task", size = 2)
...
This IAT uses categories unique to Israel: It compares the judgments toward the two main ethnicities in Isarael: Ashkenazi Israelis (originating from Europe) and Mizrahi Israelis (originating in Africa and Arab countries). Let’s inspect it further. Do people from different ethnicities have different levels of ethnicity-related bias?
...
We can see that, indeed, the IAT bias varies across the different ethnicities, and is weaker for Mizrahi Israelis. Still, all ethnicity groups in our Israeli sample (including Mizrahi Israelis) show the same direction of bias (pro-dominant).


## Summary

* Across all tasks, Israelis were biased, in their self-report and IAT scores, in favor of the dominant social groups.
* Across all tasks, the bias was much larger for the IAT than the self-report measures.
* The bias stayed pretty much the same across the 12 years of data collection.
* IAT scores: The strongest bias among Israeli participants was regarding Nationalism (Own country > USA) and the smallest bias was regarding Sexuality (Straight > Gay people).
